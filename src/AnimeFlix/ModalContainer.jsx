...
const ModalContainer = ({open, setOpen, item, isMovie}) => {
  ...
  useEffect(() => { 
    ...
    async function fetchDetails () {
      setDetails({})
   const response = await  axios.get(`https://cors.delusionz.xyz/https://api.animeflix.live/v2/episodes?id=${item.id}&dub=false`,
   {
    headers: {
      ...
      >
       <Fade in={open}>
        ...
        <Wrapper  >
          ...
          <div>
            <h2 className='modal-title'>{item?.title?.english  || item?.title?.native || item?.title}</h2>
